...
namespace SoftToss
{
    ...
    {
        const Vec3 windVel = (state.position.z >= env.windHeight) ? Vec3{env.windSpeed * std::cos(env.windDir * 1.745e-2f), env.windSpeed * std::sin(env.windDir * 1.745e-2f), 0.0} : Vec3(); // wind velocity ft/s
        ...
        if (collider)
        {
            Vec3 F_norm = normalForce(state, *collider, F_total);
            Vec3 F_fric = frictionForce(spec, state, *collider, F_total);
            Vec3 T_fric = frictionTorque(spec, state, *collider, F_fric, F_norm);
